...
# Configuración adaptada para Vercel (sistema de archivos de solo lectura)
DB_ORIGINAL = os.path.join(BASE_DIR, 'ecopulso.db')
DB = os.path.join(tempfile.gettempdir(), 'ecopulso.db')

def get_db():
    if not os.path.exists(DB) and os.path.exists(DB_ORIGINAL):
        try:
...
@app.route('/')
def index(): return send_from_directory(os.path.join(BASE_DIR, 'templates'), 'index.html')
